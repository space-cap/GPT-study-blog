...
def create_pdf_from_svgs(svg_folder, pdf_file_path, start, end):
    ...
    # 사용 가능한 페이지 너비와 높이를 가져옵니다 (여백 제외).
        ...
        if os.path.exists(svg_path):
            try:
                ...
                if drawing and drawing.width > 0 and drawing.height > 0:
                    # --- 이미지 크기 조정 로직 수정 ---
                    # 너비와 높이에 대한 스케일 비율을 각각 계산합니다.
                    width_ratio = available_width / drawing.width
                    height_ratio = available_height / drawing.height

                    # 더 작은 비율을 선택하여 이미지 전체가 페이지 안에 들어가도록 합니다.
                    scale_factor = min(width_ratio, height_ratio)

                    # 계산된 비율에 따라 너비와 높이를 조정합니다.
                ...
                elif not (drawing and drawing.width > 0 and drawing.height > 0):
                    print(
                        f"경고: {svg_path} 파일이 비어있거나 크기가 0입니다. 건너뜁니다."
                    )
                else:
