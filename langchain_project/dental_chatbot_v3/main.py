...
def run_chatbot():
    ...
    # --- 2. 챗봇 모델 및 설정 초기화 ---
    # OpenAI의 gpt-4o 모델을 사용하고, JSON 모드를 활성화하여 구조화된 데이터 추출을 용이하게 합니다.
    ...
    4. 이름과 전화번호를 모두 얻기 전까지는 절대 `PatientInfo` 형식으로 응답해서는 안 됩니다. 계속 대화를 이어가세요.
    5. 이름과 전화번호를 모두 성공적으로 얻었을 때만 `PatientInfo` 형식으로 정보를 추출하여 응답을 종료하세요.
    ...
    while True:
        try:
            ...
            # --- 4. 정보 추출 시도 ---
            # LLM에게 현재 대화 내용에서 PatientInfo를 추출할 수 있는지 확인을 요청합니다.
            # 5번의 시도 안에 정보를 추출하지 못하면 일반 대화로 전환합니다.
            try:
                patient_info = structured_llm.invoke(
                    chat_history, config={"max_retries": 5}
                )

                # 정보 추출에 성공하면 목표 달성!
                print("\n✅ [상담 정보 수집 완료]")
                print(f"  - 고객명: {patient_info.name}")
                print(f"  - 연락처: {patient_info.phone_number}")
                print(f"  - 문의 사유: {patient_info.reason or 'N/A'}")
